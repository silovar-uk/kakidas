...
- `EntryTagControl`は右側の固定タグアイコンから項目ごとの追加・変更・解除を扱う。PCではアイコン直下のポップオーバー、スマホでは下部シートを使い、カードの高さや操作列を編集UIで押し広げない。候補は同じメモ内で実在する項目タグのみで、AIによる推測は行わない。
