...
2. **本文優先、補助情報は静かに**
   ...
   - タグでまとめる表示の未完了タグ見出しでは、開閉とは別の `✎` 操作から、同じメモ・同じ区分の同名タグ（完了済みを含む）を一括で別名へ変更できる。別区分・別メモのタグは変えない。
   - 新規項目では、本文を置く前に気持ち・リンク・タグを任意で指定できる。補助情報は必要な時だけポップオーバー／下部シートで開き、保存成功後は次の項目へ持ち越さない。
...
- 未完了のタググループ見出し右側にある `✎` は、開閉ボタンと別。押すと同じメモ・同じ区分にある同名タグを、完了済み項目も含めて1トランザクションで一括変更する。変更後のタグが既存タグと同じ場合はグループを自然に統合する。別の区分・別メモは変更しない。
