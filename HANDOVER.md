...
2. **本文優先、補助情報は静かに**
   ...
   - PCでは、備考＋とリンクを完了の右側へまとめる。タグなしは同じ操作列の小さなタグアイコンから設定する。タグありは「気持ち・備考」と同じ補助行に色付きチップとして置く。スマホでは既存の操作レールを維持し、タグなしの追加もアイコンで扱う。
...
### 完全ツリー（v0.5.54・引き継ぎ資料同梱版）

以下が、この引き継ぎ書をルートへ置いた配布版の実ファイル構成である。`HANDOVER.md` と `kakidas_tree.txt` は同じ構成を記録している。`node_modules`、`dist`、`.git` を除くファイル数は **71件** である。
...
- `EntryTagControl`は項目ごとの追加・変更・解除を扱う。候補は同じメモ内で実在する項目タグのみで、AIによる推測は行わない。
- `EntryColumn`では区分ごとに`通常表示`／`タグでまとめる`を切り替えられる。表示モードは`localStorage`の`kakidas.entry-list-display-mode.<kind>`に保存し、クラウド同期しない。
- `タグでまとめる`時は、タグなし項目を先頭に通常表示し、タグ付き項目を現在の表示順を保ったままタグ別グループにする。タグが1件だけでもグループ化する。
...
- [ ] 項目タグの追加・変更・解除、通常表示での気持ち行／タグアイコン、タグでまとめる時のタグなし先頭・1件タグ・完了最下部・色付き見出し、初期折りたたみ、開閉状態の再読み込み後の維持を確認する
